...
### 목표
매일 미국장 마감 전 자동 실행 + 매매 결과 알림

### 계획
- **스케줄러**: 시스템 crontab 또는 node-cron으로 매일 ET 15:30 (한국시간 05:30) 자동 실행
- **텔레그램 알림**: 매매 판단·주문·체결 결과를 텔레그램 봇으로 전송
- **에러 알림**: 주문 실패·API 오류 시 즉시 알림

### crontab 예시 (수동 설정 가능)
```bash
# 매일 한국시간 05:30 (서머타임 기준) 실행
...
### 목표
기존 백테스트 웹 UI에 실시간 트레이딩 모니터링 페이지 추가

### 계획
- `/trading` 페이지: 현재 사이클 상태, 보유 현황, 손익 표시
- 거래 히스토리 테이블 (로그 파일 기반)
- 사이클별 수익률 차트
- 시작/중지 컨트롤 (API Route 연동)
- KIS API 잔고 조회와 내부 상태 동기화
